    ...
    public partial class Form1 : Form
    {
        ...
        //Check if everything is field
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show("Something was not inputed correctly" + "\n" + ex.Message);
                }
            }
        ...
        private void btnAddTaskCleaning_Click(object sender, EventArgs e)
        {
            ...
            Apartaments apartamentToAddTaskTo = chosenBuilding.ReturnsChosenApartament(loggedUser);
            string date = dateTimePicker1.Value.ToString("dd MMMM yyy");
            string room = comboBox_tasks.Text;
            if (comboBox_tasks.Text == "")
            {
                MessageBox.Show("Select facility");
            }
            else
            {
        ...
        private void btnAddTaskGarbage_Click(object sender, EventArgs e)
        {
            ...
            string date = dateTimePicker3.Value.ToString("dd MMMM yyy");
        ...
        private void button102_Click(object sender, EventArgs e)
        {
            ...
            string address = chosenBuilding.ToString() + " " + loggedUser.GetApartment();


            string complaint = tbDiffrentComplaint.Text;

            //Send the complaint to Admin
            lbAdminComplaints.Items.Add($"{username}, from {address} has a complaint about - {complaint}");
        ...
        private void btnComplaintAboutRoommate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (cbComplaintsDoesntClean.Checked || cbComplaintsInvitingPeople.Checked || cbComplaintsMakingNoise.Checked || cbLowHygiene.Checked)
                {
                    ...
                    //Send the complaint to Admin
                    lbAdminComplaints.Items.Add($"{username}, from {address} - Complaint about {roommate} - {complaint}");
        ...
        private void btnComplaintBrokenFacility_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //Send the complaint to Admin
                lbAdminComplaints.Items.Add($"{username}, from {address} - Complaint about broken facility - {brokenFacility}");
        ...
        private void btnAlreadyPaidAdmin_Click_1(object sender, EventArgs e)
        {
        ...
        {
            var student = (Student)lbStudentsRent.SelectedItem;
            //Check if there is a selected item in the list
            if (lbStudentsRent.SelectedItem == null)
            {
                MessageBox.Show("Please select a student for which you want to change the rent");
            ...
            else
            {
                if (student.GetRent())
                {
                    MessageBox.Show("This user already paid his rent");
                }
                else
                {
                    Announcement announcement = new Announcement($"Pay rent as soon as posible");
                    announcement.Recevers.Add(student);
            ...
            if(loggedUser != null)
            {
                ...
                foreach (Announcement announcement in Announcement.Announcements)
                {
                    foreach (Student student in announcement.Recevers)
                    {
                        ...
                        {
                            lbAnouncments.Items.Add(announcement);
                        }
                    }
                }
            }
        ...
        private void btAddNewGrocerie_Click(object sender, EventArgs e)
        {
            ...
            if (nudNewGroceriePrice.Value > 0)
            {
                ...
                else { MessageBox.Show("Alredy added"); }
            }
        ...
        private void btRemoveGrocery_Click(object sender, EventArgs e)
        {
            Grocery grocery = lboxGroceriesALL.SelectedItem as Grocery;
            if (grocery != null)
            {
        ...
        private void btBuyAllGroceries_Click(object sender, EventArgs e)
        {
            ...
            lboxGroceriesToBuy.Items.Add($"Total price: {currentApartment.totalPrice}$");
            currentApartment.finalListGroceries.Clear();
            Announcement announcement = new Announcement($"Groceries were bought for {currentApartment.totalPrice}$, on {boughtOn}. Pay your part");
            foreach( Student student in currentApartment.GetAllStudents())
            {
        ...
        private void lbAdminComplaints_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void lbAdminComplaints_DoubleClick(object sender, EventArgs e)
        {
            if (lbAnouncments.Items.Count > 0)
            {
                MessageBox.Show(lbAdminComplaints.SelectedItem.ToString());
            }
        }
        ...
        private void lboxAllStudents_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if(receivers.Contains(student))
                {
                    throw new Exception("Already in the list");
                }
                if (student != null)
                {
                    receivers.Add(student);
                    lboxReceivers.Items.Add(student.AdminTableAnnouncement);
                }
                else { throw new Exception("Nothing was selected"); }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void lboxGroceriesALL_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            catch(Exception ex) { MessageBox.Show("Nothing was selected" + ex.Message); }
        }
        ...
        {
            if (tbMessage.Text != "" || lboxReceivers.Items.Count == 0)
            {
        ...
        {
            var announcement = (Announcement)lbAnouncments.SelectedItem;
            Announcement.Announcements.Remove(announcement);
            RefreshBoxes(); 
        }
